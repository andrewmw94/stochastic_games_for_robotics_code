...
def genNeighbors(state):
    ...
    new_human_loc = Location(state.human_loc.row, state.human_loc.col)
    if isValidLoc(Location(state.human_loc.row-1, state.human_loc.col), state.human_loc, False):
        new_human_loc = Location(state.human_loc.row-1, state.human_loc.col)
    elif isValidLoc(Location(state.human_loc.row+1, state.human_loc.col), state.human_loc, False):
        new_human_loc = Location(state.human_loc.row+1, state.human_loc.col)
    elif isValidLoc(Location(state.human_loc.row, state.human_loc.col+1), state.human_loc, False):
        new_human_loc = Location(state.human_loc.row, state.human_loc.col+1)
    elif isValidLoc(Location(state.human_loc.row, state.human_loc.col-1), state.human_loc, False):
        new_human_loc = Location(state.human_loc.row, state.human_loc.col-1)
    ...
    if True:
        ...
        if isValidLoc(newLoc, state.robot_loc, state.robot_turn):
            ...
            s_prime.human_loc=new_human_loc
...
def print_robot_module(game, state_to_int_map):
    print("module robot")
    print("    rlocrow: [0..{}] init 0;".format(GRID_SIZE-1))
    print("    rloccol: [0..{}] init 0;".format(GRID_SIZE-1))
    print("    hlocrow: [0..{}] init {};".format(GRID_SIZE-1, GRID_SIZE-1))
    print("    hloccol: [0..{}] init {};".format(GRID_SIZE-1, GRID_SIZE-1))
    print("    rturn: [0..1] init 1;")
    for s in game:
        if s.robot_turn:
